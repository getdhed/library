...
func TestMigrateAppliesSchemaAndIsIdempotent(t *testing.T) {
	...
	assertRelationExists(t, db, "document_audit_events")
	...
	assertRelationExists(t, db, "idx_document_audit_events_document_created")
	assertColumnExists(t, db, "document_submissions", "source")
	assertColumnExists(t, db, "users", "is_active")
	assertColumnExists(t, db, "users", "updated_at")
